...
<script>
     ...
     * @type {Array<object>}
     */
     export let listJsonObject;
     /**
     * @type {String}
     */
     export let header = '';

    function elementHTMLToPDF() {
        dataToPdf(listJsonObject, header)        
    }
</script>


<button class="w-1/2 h-full m-6 p-2 pl-3 pr-3 bg-blue-700 font-semibold text-white rounded-md flex items-center justify-center hover:bg-blue-800" on:click={elementHTMLToPDF}>
