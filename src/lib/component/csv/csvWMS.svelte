<script>
    import {wmsListToCSV} from '$lib/component/csv/gerarCSV'
    /**
     * @type {any}
     */
     export let wmsArrayToCSV;

     const csvGenerate = () => {
        wmsListToCSV(wmsArrayToCSV)     
    } 
...
<button class="w-1/2 m-6 p-2 pl-3 pr-3 bg-blue-700 font-semibold text-white rounded-md flex items-center justify-center hover:bg-blue-800" on:click={csvGenerate}>
...
<!--
<button class="flex m-4 p-2 bg-blue-500 text-white rounded-full" id="buttonRemove" on:click={csvGenerate}>
    Gerar CSV
</button>
--->