<script>
    ...
    import {countTotalLayerWFS, countWFSProcessado, keywordWFSCountByName, allWFSKeywords} from '$lib/store/storeWFS'
  ...
  import {keywordsToCSV} from '$lib/component/csv/gerarCSV'
    ...
    async function btnSearchClicked() {
        ...
        unsubscribe = keywordWFSCountByName.subscribe((value) => {
...
<form class="m-2">
    <div class="flex items-center flex-col sm:flex-row mb-1 text-sm font-medium text-gray-900 dark:text-gray-400">
        ...
        <p class="ml-auto text-sm ">Qtd de palavras chaves: {$allWFSKeywords.length}</p>
    ...
    <div class="mt-2 w-full h-14 p1 flex flex-col md:flex-row">
...
<div class = "">
        ...
        {#each selectedCatalogs as objIdDescricaoIri}
            <WFSKeywordCard objIdDescricaoIri={objIdDescricaoIri} ></WFSKeywordCard>
        {/each}
        <div class = "m-2 grid gap-2 md:grid-cols-4 grid-cols-2">
            {#each Object.entries($keywordWFSCountByName) as keyCount, i (i)}
